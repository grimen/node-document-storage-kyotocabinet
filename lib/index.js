...
function KyotoCabinet () {
  ...
  self.options.server.db = [self.options.server.db, self.options.server.extension].join('.').replace('..', '.');
}
...
KyotoCabinet.prototype.connect = function() {
  ...
  self._connect(function() {
    ...
    fs.mkdir(path.dirname(self.options.server.db), self.options.mode, true, function(err) {
      if (err) {
        self.emit('error', err);
      }
      self.emit('ready');
...
// #key (key)
KyotoCabinet.prototype.key = function(key) {
  var self = this;
  var _key = [key].join('/');
  return _key;
};
...
KyotoCabinet.prototype.set = function() {
  ...
  self._set(arguments, function(key_values, options, done, next) {
    self.client.open(self.options.server.db, self.options.client.mode, function(_err) {
      var store = this;
...
KyotoCabinet.prototype.del = function() {
  ...
  self._del(arguments, function(keys, options, done, next) {
    self.client.open(self.options.server.db, self.options.client.mode, function() {
      var store = this;
      var res = Object.extended({});
